...
namespace qcview {
QCefViewBrowserHandler::QCefViewBrowserHandler(QCefView::Delegate* pDelegate)
  : is_closing_(false)
...
QCefViewBrowserHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser)
{
  ...
  if (!main_browser_) {
    ...
    CefPostTask(TID_UI, CefCreateClosureTask(base::BindOnce(&CefBrowserHost::SetFocus, browser->GetHost().get(), true)));
  }

  SchemeHandlerFactory::recordBrowserAndDelegate(browser, pQCefViewDelegate_);
...
QCefViewBrowserHandler::OnBeforeClose(CefRefPtr<CefBrowser> browser)
{
  ...
  SchemeHandlerFactory::removeBrowserAndDelegate(browser);
...
QCefViewBrowserHandler::NotifyTakeFocus(bool next)
{
  if (!CefCurrentlyOn(TID_UI)) {
    ...
    CefPostTask(TID_UI, CefCreateClosureTask(base::BindOnce(&QCefViewBrowserHandler::NotifyTakeFocus, this, next)));
    return;
  }
...
QCefViewBrowserHandler::NotifyDragRegion(const std::vector<CefDraggableRegion> regions)
{
  if (!CefCurrentlyOn(TID_UI)) {
    // Execute this method on the main thread.
    CefPostTask(TID_UI, CefCreateClosureTask(base::BindOnce(&QCefViewBrowserHandler::NotifyDragRegion, this, regions)));
    return;
  }
