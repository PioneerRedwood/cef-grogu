...
namespace qcview {
SchemeHandler::SchemeHandler(QCefView::Delegate* pDelegate)
...
std::map<void*, QCefView::Delegate*> SchemeHandlerFactory::mapBrowser2Delegate_;

std::mutex SchemeHandlerFactory::mtxMap_;

void SchemeHandlerFactory::recordBrowserAndDelegate(CefRefPtr<CefBrowser> browser, QCefView::Delegate* pDelegate)
{
  if (!browser)
    return;
...
void SchemeHandlerFactory::removeBrowserAndDelegate(CefRefPtr<CefBrowser> browser)
{
  std::lock_guard<std::mutex> lock(mtxMap_);
...
{
  QCefView::Delegate* pDelegate = nullptr;
