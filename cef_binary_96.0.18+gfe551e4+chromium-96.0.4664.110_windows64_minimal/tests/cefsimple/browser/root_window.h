...
#ifndef CEF_TESTS_CEFSIMPLE_BROWSER_ROOT_WINDOW_H_
#define CEF_TESTS_CEFSIMPLE_BROWSER_ROOT_WINDOW_H_
#pragma once
...
#include "tests/cefsimple/client_types.h"
#include "tests/cefsimple/image_cache.h"
...
// 루트창이 어떻게 생성될 지에 대한 설정으로 사용됩니다.
// 항상 위, 제어할 수 있는지(최소화, 닫기 등), osr, 확장앱을 호스팅하는지, 초기 숨김 상태인지
...
using ExtensionSet = std::set<CefRefPtr<CefExtension>>;
...
// 브라우저 프로세스 내의 최상위 네이티브 창을 나타냅니다.
class RootWindow
 ...
 public:
  ...
  class Delegate {
   public:
    ...
    // 브라우저에 있는 CefRequestContext를 생성하는데 호출
    ...
    // 리소스에 등록된 테스트 ID에 따라 테스트 실행을 호출
